...
export default function ClientTrainingBlocks({ client }) {
  ...
  // Block create/edit lives in the client's own Goals wizard, so enter the client's account (view-as)
  // and land on Goals with the wizard opened — fresh (planBlock=1) or resumed on a block (editBlock=id).
  const openClientWizard = async (query) => {
    setError("");
  ...
  return (
    <Paper variant="outlined" sx={{ p: 2, mb: 2 }}>
      <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 1 }}>
        ...
        <Button size="small" variant="outlined" onClick={() => openClientWizard("planBlock=1")}>Plan a Training Block</Button>
      </Stack>
      {loading ? (
            ...
            return (
              <Box key={b._id} sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 1 }}>
                ...
                <Stack direction="row" spacing={1} sx={{ flexShrink: 0 }}>
                  <Button size="small" onClick={() => openClientWizard(`editBlock=${b._id}`)}>
                    Edit block
                  </Button>
                  {prog ? (
