...
// Hoisted to module scope on purpose — same fix as the Clients page. Defined inside
// Trainers(), this was a new component type on every parent render, so React remounted every
// card and wiped its local state: the permissions panel and the delete dialog closed
// themselves whenever the parent re-rendered.
const RelationshipTrainerCard = ({
  trainer,
  dispatch,
  user,
  creditByTrainer,
        ...
        <Grid container spacing={2} sx={{ padding: "15px" }}>
          ...
          <Grid container size={12} sx={{ justifyContent: "center", paddingBottom: "15px" }}>
            {loading ? (
              ...
              myTrainers?.map((t) => (
                <RelationshipTrainerCard
                  key={t.trainer}
                  trainer={t}
                  dispatch={dispatch}
                  user={user}
                  creditByTrainer={creditByTrainer}
