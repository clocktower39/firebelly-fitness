...
// Hoisted to module scope on purpose. Defined inside Clients(), this was a NEW component
// type on every parent render, so React unmounted and remounted every card whenever the
// parent re-rendered — wiping each card's local state. Saving a setting calls
// setStatusMessage/dispatch, which re-renders the parent, which collapsed the settings panel
// mid-edit. A stable identity keeps `expanded` (and the More menu, and the delete dialog)
// alive until the trainer closes it.
const ClientCard = ({
  clientRelationship,
  isOnline,
  dispatch,
              ...
              <>
                ...
                >
                  ...
                  >
                    ...
                    <MenuItem
                      component={Link}
                      to={`/session-history?client=${clientRelationship.client._id}`}
                      onClick={() => setMenuAnchor(null)}
                    >
                      Session History
                    </MenuItem>
        ...
        {filteredClients.length > 0 ? (
          filteredClients.map((clientRelationship) => (
            <ClientCard
              ...
              setRecapClient={setRecapClient}
              handleAcceptRelationship={handleAcceptRelationship}
              handleViewAsClient={handleViewAsClient}
              handleOpenCalendar={handleOpenCalendar}
              handleOpenGoals={handleOpenGoals}
            />
          ))
        ) : (
